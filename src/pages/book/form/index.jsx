	...
	renderSummary(formValues) {
		...
		if (Service_Type_ID && No_Of_Staff) {
			...
			serviceTypeList.data.map((data) => {
				...
				if (data.timePerStaff && data._id == Service_Type_ID) Service = data;
			});
			if (Service) {
				return (
					<div className="bg-light p-3">
						...
						<small className="text-black-50">&#8377; {parseFloat(Service.amount)}/hour/staff</small>
						<hr />
						Total{' '}
				...
				<Form onSubmit={this.props.handleSubmit(this.props.getValues.bind(this))}>
					...
					<Row>
						...
						<Col xs={12} md={4}>
							...
							<FormGroup>
								<FormField placeholder="Date" name="serviceDate" type="date" validate={[ required ]} />
							</FormGroup>
							<FormGroup>
								<FormField placeholder="Time" name="serviceTime" type="time" validate={[ required ]} />
							</FormGroup>
						</Col>
						<Col xs={12} md={4} className="align-self-top">
